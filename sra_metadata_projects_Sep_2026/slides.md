...
Two methods recover metadata from the archive alone. No paper, no model.

::: cards cols=2 gap=24px size=md border=top
### Copy from sibling samples {accent=sky}

The most frequent value of that field elsewhere in the same project.

### Copy from the sample title {accent=sky}
...
<!--
Where the title is autogenerated ("Model organism or animal sample from Rattus
norvegicus") both fail and the paper is the only source. Second prerequisite
for paper-based projects: how often is the value in the paper at all? That is
the ceiling on extraction. Preliminary figures in analysis/PROJECTS-2026-08-26.md
rest on 348 cells and are not firm.
-->
